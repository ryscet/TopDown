...
def storeEEGinHDF():
    ...
    all_eeg_names= [name for name in eeg_names if '1_TD.mat' in name]
    store = HDFStore(h_path +"td_before_database.hdf5")
...
def SAVE_ChangeDictOrder():
    '''Change the nesting order for the final HDF database - insted of correct/attention, it will go attention/present/correct etc'''
    
    
    h_path = "/Users/ryszardcetnarski/Desktop/Nencki/TD/HDF/"
    all_eeg_names= [name for name in eeg_names if '1_TD.csv' in name]
    
    store = HDFStore(h_path +"td_before_database.hdf5")
      
    
    for _data, recording in zip(_processedEvents, all_eeg_names):
        print('I')
        sname = recording.rfind("/") +1
        subId = recording[sname:-4].replace("-", "_") 
...
def LoadEvents():
    ...
    mergedDict = MakeDict()
    mat_events = [name for name in events_names if '1_TD_EVENTS' in name]
    allEvents = []
